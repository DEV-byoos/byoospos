...
class Config extends Secure_Controller
{
	...
	private function _licenses()
	{
		...
		foreach($dir as $fileinfo)
		{
			...
			if($fileinfo->isFile())
			{
				if($fileinfo->getExtension() == 'version')
				{
					...
					$basename = 'public/license/' . $fileinfo->getBasename('.version');
		...
		if($composer)
		{
			...
			$file = file_get_contents('public/license/composer.LICENSES');
		...
		if($bower)
		{
			...
			$file = file_get_contents('public/license/bower.LICENSES');
			$array = json_decode($file, TRUE);
	...
	private function _handle_logo_upload()
	{
		...
		// load upload library
		$config = array('upload_path' => './public/uploads/',
